(function () {
  const deck = window.CLAUDE_DECK || window.CLAUDE_CORE_DECK;
  const state = {
    index: 0,
    mapOpen: true,
    notesOpen: true,
  ...
  function codeVisual(slide) {
    const code = slide.code || [
      "claude",
      "/context",
      "Read the repo, propose a 3-step plan, do not edit yet.",
      "/usage"
    ];
    return `
      <div class="terminal">
        <div class="window-bar">
          ...
          <span class="window-title">terminal · claude code</span>
        </div>
        <pre>${code.map((line, index) => {
          const cls = index === 0 ? "prompt" : line.startsWith("#") ? "comment" : "";
  ...
  function visualFor(slide) {
    ...
    if (slide.type === "code" || slide.code) return codeVisual(slide);
    ...
    return cards([["Idea clave", slide.body || slide.subtitle || "Claude Code se aprende mejor con memoria, limites y verificacion."]]);
  }
  ...
  if (!deck?.slides?.length) {
    slideFrame.innerHTML = "<article class=\"slide full\"><h2>No hay slides cargadas</h2></article>";
    return;
  }
