# Prompts iniciales

## Prompt 1 - Briefing verificable

Actua como asistente de copilot. Antes de editar, analiza la tarea:

- Objetivo: Revisar PR generado por agente
- Contexto: Aplicar politica de PRs generadas por IA: evidencia, tests, riesgo y responsabilidad humana.
- Superficie: GitHub PR
- Permisos: review-only
- Modelo recomendado: Copilot code review + humano
- Coste estimado: Medio: review y re-review

Devuelve:

1. Archivos o fuentes que revisarias.
2. Plan de 3 pasos.
3. Riesgos.
4. Criterio de terminado.

## Prompt 2 - Ejecucion acotada

Ejecuta solo el plan aprobado. No cambies comportamiento no relacionado. Cierra con:

- resumen del cambio
- evidencia
- checks ejecutados
- coste/modelo/permisos usados

