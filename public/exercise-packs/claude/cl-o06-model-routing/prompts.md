# Prompts iniciales

## Prompt 1 - Briefing verificable

Actua como asistente de claude. Antes de editar, analiza la tarea:

- Objetivo: Routing de modelos por coste
- Contexto: Diseña una tabla que asigne Haiku, Sonnet u Opus segun tarea, riesgo y coste esperado.
- Superficie: Model picker
- Permisos: governance
- Modelo recomendado: Haiku + Sonnet + Opus
- Coste estimado: bajo: diseño de politica

Devuelve:

1. Archivos o fuentes que revisarias.
2. Plan de 3 pasos.
3. Riesgos.
4. Criterio de terminado.

## Prompt 2 - Ejecucion acotada

Ejecuta solo el plan aprobado. No cambies comportamiento no relacionado. Cierra con:

- resumen del cambio
- evidencia
- checks ejecutados
- coste/modelo/permisos usados

