...
import { byIds, getCopilotExercises, getCopilotWorkshop } from '@/lib/content'
...
export default async function CopilotWorkshopPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const [workshop, allExercises] = await Promise.all([
    getCopilotWorkshop(id),
    getCopilotExercises(),
  ])
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(135deg,#050505_0%,#10131a_44%,#14170f_100%)] text-zinc-50">
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_18%_8%,rgba(56,189,248,0.16),transparent_30%),radial-gradient(circle_at_86%_0%,rgba(244,114,182,0.12),transparent_28%),linear-gradient(rgba(255,255,255,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.035)_1px,transparent_1px)] bg-[size:auto,auto,40px_40px,40px_40px]" />
      <div className="relative mx-auto w-full max-w-6xl px-4 py-6 md:px-8 md:py-10">
        ...
        <section className="rounded-lg border border-white/10 bg-white/[0.045] p-6 shadow-2xl shadow-black/25 backdrop-blur-xl md:p-8">
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            {workshop.videoSlugs[0] ? (
              <Link href={`/player/${workshop.videoSlugs[0]}`} className="inline-flex min-h-9 items-center gap-2 rounded-md border border-sky-300/60 bg-sky-300 px-3 text-xs font-semibold text-zinc-950 transition hover:bg-sky-200">
                <Play className="h-4 w-4" />
                Abrir primer video
              </Link>
            ) : null}
            {exercises[0] ? (
              <Link href={`/tracks/copilot/exercises/${exercises[0].id}`} className="inline-flex min-h-9 items-center gap-2 rounded-md border border-white/10 bg-white/[0.05] px-3 text-xs font-semibold text-zinc-100 transition hover:border-white/25">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <section className="rounded-lg border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
            <h2 className="flex items-center gap-2 text-xl font-semibold"><CheckCircle2 className="h-5 w-5 text-sky-200" />Outputs esperados</h2>
        ...
        {workshop.instructorPackHref ? (
          <section className="mt-10 space-y-4">
            <h2 className="flex items-center gap-2 text-2xl font-semibold"><FileText className="h-6 w-6 text-sky-200" />Pack instructor</h2>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {exercises.map(exercise => (
              <Link key={exercise.id} href={`/tracks/copilot/exercises/${exercise.id}`} className="rounded-lg border border-white/10 bg-white/[0.04] p-5 transition hover:border-white/25 hover:bg-white/[0.06]">
                <div className="flex flex-wrap gap-2"><Pill>{exercise.level}</Pill><Pill>{exercise.difficulty}</Pill><Pill><Clock className="mr-1.5 h-3.5 w-3.5 text-sky-200" />{exercise.durationMin} min</Pill></div>
                ...
                <span className="mt-5 inline-flex items-center gap-2 text-xs font-semibold text-sky-100">Abrir reto<ArrowRight className="h-4 w-4" /></span>
              </Link>
            ))}
          </div>
        </section>

        <FeedbackWidget kind="workshop" vendor="copilot" targetId={workshop.id} />
      </div>
    </main>
  )
}
