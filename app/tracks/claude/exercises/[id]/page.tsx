...
import { byIds, getClaudeCourse, getClaudeExercise, getClaudeMaterials } from '@/lib/content'
...
export default async function ClaudeExercisePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const [exercise, allMaterials] = await Promise.all([
    getClaudeExercise(id),
    getClaudeMaterials(),
  ])

  if (!exercise) notFound()

  const course = await getClaudeCourse(exercise.courseId)
    ...
    <main className="min-h-screen bg-[linear-gradient(135deg,#050505_0%,#15110f_44%,#10151a_100%)] text-zinc-50">
      ...
      <div className="relative mx-auto w-full max-w-6xl px-4 py-6 md:px-8 md:py-10">
        <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
          ...
          {course ? (
            <Link href={`/tracks/claude/courses/${course.id}`} className="inline-flex min-h-9 items-center gap-2 rounded-md border border-white/10 bg-white/[0.04] px-3 text-xs font-semibold text-zinc-100 transition hover:border-white/25">
        ...
        <section className="rounded-lg border border-white/10 bg-white/[0.045] p-6 shadow-2xl shadow-black/25 backdrop-blur-xl md:p-8">
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            {exercise.videoSlug ? (
              <Link href={`/player/${exercise.videoSlug}`} className="inline-flex min-h-9 items-center gap-2 rounded-md border border-rose-300/60 bg-rose-300 px-3 text-xs font-semibold text-zinc-950 transition hover:bg-rose-200">
        ...
        <ExercisePackPanel vendor="claude" exerciseId={exercise.id} />

        <div className="mt-6 grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <section className="rounded-lg border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
            <h2 className="flex items-center gap-2 text-xl font-semibold"><Wrench className="h-5 w-5 text-rose-200" />Tareas</h2>
            <ol className="mt-4 space-y-3">
              {exercise.tasks.map((task, index) => (
                <li key={task} className="grid grid-cols-[2rem_1fr] gap-3 rounded-md border border-white/10 bg-black/20 p-3 text-sm leading-6 text-zinc-300">
                  <span className="font-mono text-rose-200">{String(index + 1).padStart(2, '0')}</span>
                  <span>{task}</span>
                </li>
              ))}
            </ol>
        ...
        <section className="mt-6 rounded-lg border border-rose-300/20 bg-rose-300/[0.055] p-5 backdrop-blur-xl">
          <h2 className="flex items-center gap-2 text-xl font-semibold"><CheckCircle2 className="h-5 w-5 text-rose-200" />Solucion esperada</h2>
        ...
        <FeedbackWidget kind="exercise" vendor="claude" targetId={exercise.id} />
      </div>
    </main>
  )
}
