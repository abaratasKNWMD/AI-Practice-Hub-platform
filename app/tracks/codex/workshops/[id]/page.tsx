...
export default async function WorkshopPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(135deg,#050505_0%,#111411_44%,#1b1711_100%)] text-zinc-50">
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_18%_8%,rgba(244,114,182,0.14),transparent_30%),radial-gradient(circle_at_86%_0%,rgba(16,185,129,0.13),transparent_28%),linear-gradient(rgba(255,255,255,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.035)_1px,transparent_1px)] bg-[size:auto,auto,40px_40px,40px_40px]" />
      <div className="relative mx-auto w-full max-w-6xl px-4 py-6 md:px-8 md:py-10">
        ...
        <section className="rounded-lg border border-white/10 bg-white/[0.045] p-6 shadow-2xl shadow-black/25 backdrop-blur-xl md:p-8">
          <div className="flex flex-wrap gap-2">
            <Pill>{workshop.duration}</Pill>
            <Pill>{workshop.challengeIds.length} retos</Pill>
            <Pill>{workshop.videoSlugs.length} videos</Pill>
          </div>
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            {workshop.videoSlugs[0] ? (
              <Link
                href={`/player/${workshop.videoSlugs[0]}`}
            ...
            {exercises[0] ? (
              <Link
                href={`/tracks/codex/exercises/${exercises[0].id}`}
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <section className="rounded-lg border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
            <h2 className="flex items-center gap-2 text-xl font-semibold">
              ...
              Notas para facilitar
            </h2>
            <div className="mt-4 space-y-3">
              {workshop.facilitatorNotes.map(note => (
                <p key={note} className="rounded-md border border-white/10 bg-black/20 p-3 text-sm leading-6 text-zinc-300">
                  {note}
                </p>
              ))}
            </div>
          ...
          <section className="rounded-lg border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {workshop.outputs.map(output => (
                <p key={output} className="rounded-md border border-white/10 bg-black/20 p-3 text-sm leading-6 text-zinc-300">
                  {output}
                </p>
              ))}
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-4">
              {[
                ...
                <Link
                  key={href}
                  href={href}
                  className="rounded-lg border border-white/10 bg-white/[0.04] p-4 text-sm font-semibold text-zinc-100 transition hover:border-white/25 hover:bg-white/[0.06]"
                >
                  {label}
                </Link>
              ))}
            ...
            {exercises.map(exercise => (
              ...
              >
                <div className="flex flex-wrap gap-2">
                  ...
                  <Pill>
                    <Clock className="mr-1.5 h-3.5 w-3.5 text-emerald-200" />
        ...
        <FeedbackWidget kind="workshop" vendor="codex" targetId={workshop.id} />
      </div>
    </main>
  )
}
