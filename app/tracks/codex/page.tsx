...
import { CodexTrackClient } from '@/components/hub/codex-track-client'
import { getCodexCourses, getCodexExercises, getCodexMaterials, getCodexWorkshops } from '@/lib/content'

export default async function CodexTrackPage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[linear-gradient(135deg,#050505_0%,#111411_44%,#1b1711_100%)] text-zinc-50">
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_18%_8%,rgba(16,185,129,0.18),transparent_30%),radial-gradient(circle_at_82%_0%,rgba(251,191,36,0.12),transparent_28%),linear-gradient(rgba(255,255,255,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.035)_1px,transparent_1px)] bg-[size:auto,auto,40px_40px,40px_40px]" />
        ...
        <header className="mb-8 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden text-right sm:block">
              <p className="text-sm font-semibold text-zinc-50">Codex Practice Hub</p>
              <p className="text-xs text-zinc-500">AI Practice Hub v1</p>
            </div>
          </div>
        </header>

        <CodexTrackClient courses={courses} exercises={exercises} workshops={workshops} materials={materials} />
      </div>
    </main>
  )
}
