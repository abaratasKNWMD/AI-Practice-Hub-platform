...
interface ClaudeTrackClientProps {
  courses: CourseMeta[]
  ...
  return (
    <button
      ...
      className={`min-h-9 rounded-md border px-3 text-xs font-medium transition ${
        active
          ? 'border-rose-300/60 bg-rose-300/15 text-rose-100 shadow-[0_0_24px_rgba(251,113,133,0.16)]'
  ...
  return (
    <Link
      ...
      className={`inline-flex min-h-9 items-center justify-center gap-2 rounded-md border px-3 text-xs font-semibold transition ${
        primary
          ? 'border-rose-300/60 bg-rose-300 text-zinc-950 hover:bg-rose-200'
...
function ClaudeConsoleVisual() {
  return (
    <div className="overflow-hidden rounded-lg border border-white/10 bg-[#11100f] shadow-2xl shadow-black/30">
      <div className="flex items-center justify-between border-b border-white/10 bg-[#1a1715] px-4 py-3">
        <div className="flex gap-1.5">
          <span className="h-3 w-3 rounded-full bg-red-400" />
          <span className="h-3 w-3 rounded-full bg-amber-300" />
          <span className="h-3 w-3 rounded-full bg-emerald-300" />
        </div>
        <span className="text-xs font-mono text-zinc-500">claude.code.practice</span>
      </div>
      <div className="grid min-h-[26rem] grid-cols-[3.3rem_1fr_18rem]">
        <div className="border-r border-white/10 bg-[#0c0b0b] p-3">
        ...
        <div className="bg-[#11100f] p-5 font-mono text-sm">
          <p className="text-zinc-500">CLAUDE.md</p>
          <pre className="mt-6 whitespace-pre-wrap leading-7 text-zinc-300">
            <span className="text-rose-200"># Project memory</span>{'\n\n'}
            <span className="text-zinc-500">{'// stable context, not a giant prompt'}</span>{'\n'}
            {'- Use pnpm for commands'}{'\n'}
            {'- Read tests before patching'}{'\n'}
            {'- Run the cheapest relevant check'}{'\n\n'}
            <span className="text-cyan-200">/context</span> <span className="text-zinc-500">before enabling MCP</span>{'\n'}
            <span className="text-amber-200">/usage</span> <span className="text-zinc-500">before long loops</span>
          </pre>
        </div>
        <div className="border-l border-white/10 bg-[#151313]/90 p-4">
          <div className="rounded-md border border-rose-300/20 bg-rose-300/10 p-3">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-rose-100">Claude control</p>
            <p className="mt-3 text-sm leading-6 text-zinc-300">
              Memory para contexto estable. Skills para procesos. Hooks para reglas que se ejecutan.
            </p>
          </div>
          <div className="mt-4 space-y-2">
            {[
              ...
              <div key={kind} className="flex items-center justify-between rounded-md border border-white/10 bg-black/20 px-3 py-2 text-xs text-zinc-300">
                <span>{label}</span>
                <span className="font-mono text-rose-200">0{index + 1}</span>
              </div>
            ))}
          </div>
        </div>
...
export function ClaudeTrackClient({ courses, exercises, workshops, materials }: ClaudeTrackClientProps) {
    ...
    <div className="space-y-14">
      <section className="grid gap-4 xl:grid-cols-[0.92fr_1.08fr]">
        <div className="rounded-lg border border-white/10 bg-white/[0.045] p-5 shadow-2xl shadow-black/20 backdrop-blur-xl md:p-6">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <MetaPill icon={Wallet} label="context + cost" />
          </div>
          <h1 className="mt-7 max-w-3xl text-4xl font-semibold tracking-tight text-zinc-50 md:text-6xl">
            Claude Code Practice
          </h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-300">
            Memoria, skills, subagentes, hooks, MCP, plugins y Agent SDK para equipos que quieren
            operar Claude como plataforma, no como chat improvisado.
          </p>
          <div className="mt-7 flex flex-wrap gap-2">
            ...
            <ActionLink href="/player/cl-masterclass-10m-claude-operating-model">
      ...
      <section id="cursos" className="space-y-5">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-rose-200">Ruta Claude</p>
            <h2 className="mt-2 text-2xl font-semibold text-zinc-50">Cursos autoconsumibles CL0-CL4</h2>
          </div>
          <p className="max-w-xl text-sm leading-6 text-zinc-400">
            La ruta aterriza Claude Code con materiales que se copian a repos reales: memoria, rules, skills,
            subagentes, hooks, MCP, plugin y SDK.
          </p>
        </div>
          ...
          {courses.map(course => (
            <article key={course.id} className="rounded-lg border border-white/10 bg-white/[0.045] p-5 shadow-xl shadow-black/10 backdrop-blur-xl">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                <ActionLink href={`/tracks/claude/courses/${course.id}`} primary>
                  Abrir curso
      ...
      <section id="ejercicios" className="space-y-5">
        <div className="flex flex-col justify-between gap-3 xl:flex-row xl:items-end">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-cyan-200">Practice platform</p>
            <h2 className="mt-2 text-2xl font-semibold text-zinc-50">Ejercicios Claude filtrables</h2>
          </div>
          <label className="text-xs text-zinc-400 xl:w-[28rem]">
            ...
            <select
              ...
              className="mt-1 h-9 w-full rounded-md border border-white/10 bg-zinc-950 px-3 text-xs text-zinc-100 outline-none focus:border-rose-300/50"
            >
              {surfaces.map(item => (
          ...
          {filteredExercises.map(exercise => {
            ...
            return (
              <article key={exercise.id} className="rounded-lg border border-white/10 bg-white/[0.04] p-5 shadow-xl shadow-black/10 backdrop-blur-xl">
                <div className="flex flex-wrap gap-2">
                  <MetaPill icon={Brain} label={course?.code ?? exercise.level} />
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  <ActionLink href={`/tracks/claude/exercises/${exercise.id}`} primary>
                    Empezar ejercicio
                    <ArrowRight className="h-4 w-4" />
                  </ActionLink>
                  {exercise.videoSlug ? (
      ...
      <section id="workshops" className="space-y-5">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.28em] text-amber-200">Workshops</p>
          <h2 className="mt-2 text-2xl font-semibold text-zinc-50">Retos de memoria, operador, hooks y SDK</h2>
        </div>
        <div className="grid gap-4 lg:grid-cols-4">
          {workshops.map(workshop => (
            <article key={workshop.id} className="rounded-lg border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
              ...
              <div className="mt-5">
                <ActionLink href={`/tracks/claude/workshops/${workshop.id}`} primary>
                  Abrir workshop
                  <ArrowRight className="h-4 w-4" />
                </ActionLink>
              </div>
      ...
      <section id="materiales" className="space-y-5">
        <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-emerald-200">Materiales</p>
            <h2 className="mt-2 text-2xl font-semibold text-zinc-50">Claude pack listo para copiar</h2>
          </div>
          <p className="max-w-xl text-sm leading-6 text-zinc-400">
            Incluye CLAUDE.md, settings, rules, skills, subagentes, hooks, MCP, plugin team-review,
            docs de governance y ejemplo Agent SDK.
          </p>
        </div>
          ...
          {materials.map(material => (
            <Link key={material.id} href={material.href} className="group rounded-lg border border-white/10 bg-white/[0.035] p-4 transition hover:border-white/25 hover:bg-white/[0.06]">
              <div className="flex items-center justify-between gap-3">
                <span className="rounded-md border border-white/10 bg-black/20 px-2 py-1 text-[11px] text-rose-100">
                  {material.type}
                </span>
